...
__declspec(align(16)) struct CrossMsg final {
	...
	__declspec(noalias) __forceinline void serialize(Archive& ar,
...
__declspec(align(16)) struct Peano2DMap final sealed{
	...
	__forceinline Peano2DMap(int L,
													 float _a,
													 float _b,
													 float _c,
													 float _d,
													 unsigned st) noexcept
			: levels(L),
				a(_a),
				b(_b),
				c(_c),
...
__declspec(align(16)) struct Interval1D final sealed{
	...
	__declspec(noalias) __forceinline Interval1D(float _x1,
																							 float _x2,
																							 float _y1,
																							 float _y2,
																							 float _N) noexcept
...
#pragma loop ivdep
		...
		__m256 vinvm = _mm256_rcp_ps(vm);
		vinvm = _mm256_mul_ps(
			vinvm, _mm256_fnmadd_ps(vm, vinvm, _mm256_set1_ps(2.0f)));
...
#pragma loop ivdep
	...
	while (i < n) {
		...
		const float mi =
			fmaf(GF, fmaf(-p->x1, 1.0f, p->x2), fmaf(p->M, alpha, 0.0f));
		arr[i]->R = fmaf(1.0f / mi,
			p->quadratic_term,
			fmaf(mi, p->N_factor, p->ordinate_factor));
		++i;
	}
}
...
__declspec(align(16)) struct IntervalND final sealed{
	...
	__declspec(noalias) __forceinline IntervalND(float _x1,
																							 float _x2,
																							 float _y1,
																							 float _y2) noexcept
			: x1(_x1),
				...
				ordinate_factor(
						fmaf(fmaf(-y1, 1.0f, -y2), 2.0f, 0.0f)),
				N_factor(0),
				quadratic_term(0),
				M(0),
				R(0),
	...
	__declspec(noalias) __forceinline void compute_span_level(
			const struct MortonND& map) noexcept;

	__declspec(noalias) __forceinline void set_metric(float d_alpha) noexcept {
		N_factor = d_alpha;
		quadratic_term = fmaf(1.0f / N_factor,
													fmaf(delta_y, delta_y, 0.0f),
													0.0f);
...
static __declspec(noalias) __forceinline float step(float _m,
	...
	if (_N == 2.0f) {
		return fmaf(fmaf(fmaf(fmaf(1.0f / fmaf(_m, _m, 0.0f), sign_mult, 0.0f),
			fmaf(fmaf(diff, diff, 0.0f), _r, 0.0f),
			0.0f),
			1.0f,
			fmaf(x1, 1.0f, x2)),
			0.5f,
	...
	return fmaf(
		fmaf(
			fmaf(
				fmaf(1.0f / fast_pow_int(_m, (int)_N), sign_mult, 0.0f),
				fast_pow_int(fabsf(diff), (int)_N),
				0.0f),
			_r,
			0.0f),
		1.0f,
...
__declspec(align(16)) struct MortonND final sealed{
	...
	static __declspec(noalias) __forceinline unsigned long long make_mask(
			int dim,
			int Lc,
			int d) noexcept {
		unsigned long long m = 0ull;
		unsigned long long bitpos = (unsigned long long)d;
		int b = 0;
#pragma loop ivdep
		while (b < Lc) {
			m |= 1ull << bitpos;
	...
	__declspec(noalias) __forceinline MortonND(int D,
																						 int L,
																						 const float* lows,
																						 const float* highs,
																						 const MortonCachePerRank& mc)
			: dim(D),
				levels(L),
				eff_levels((std::max)(1,
															(int)(63 / (D ? D : 1)))),
				extra_levels((L > eff_levels) ? (L - eff_levels) : 0),
				chunks((extra_levels > 0)
									 ? (1 + (extra_levels + eff_levels - 1) / eff_levels)
									 : 1),
				low(lows, lows + D),
				high(highs, highs + D),
				step(D, 0.0f),
...
#pragma loop ivdep
		...
		pextMaskChunks.resize((size_t)chunks * (size_t)dim);
		...
		while (c < chunks) {
			const int Lc =
					(c == 0) ? (std::min)(eff_levels, remaining)
									 : (std::min)(eff_levels, remaining);
			chunk_bits[c] = Lc;
			remaining -= Lc;

			const unsigned long long baseC =
					(unsigned long long)1 << (dim * Lc);
			chunk_bases[c] = baseC;
...
#pragma loop ivdep
		...
		scale = (unsigned long long)1 << (dim * chunk_bits[0]);
	}

	__declspec(noalias) __forceinline float block_diameter(
			unsigned long long i1,
			unsigned long long i2) const noexcept {
		if (i1 > i2) {
			std::swap(i1, i2);
		}
...
#pragma loop ivdep
	...
	__declspec(noalias) __forceinline void map01ToPoint(
			float t,
		...
		while (c < chunks) {
			...
			const float scaled = t * (float)baseC;
			unsigned long long idxc =
					(scaled >= (float)baseC)
							? (baseC - 1ull)
				...
				if (inv_shift >= 0) {
					...
					if (chunk_bits[c] < 63) {
						const unsigned long long take =
								((unsigned long long)1 << chunk_bits[c]) - 1ull;
						invMaskSegment =
								(invMask[pd] >> inv_shift) & take;
					}
					bits ^= invMaskSegment;
				}
...
#pragma loop ivdep
	...
	__declspec(noalias) __forceinline float pointToT(
			const float* __restrict q) const noexcept {
...
#pragma loop ivdep
		while (d < dim) {
			...
			if (bitsCoarse < 63) {
				const unsigned long long take =
						((unsigned long long)1 << bitsCoarse) - 1ull;
				invMask0 =
		...
		if (use_gray) {
			idx0 = (unsigned long long)gray_decode(idx0);
		}

		return fmaf(1.0f / (float)scale,
								fmaf((float)idx0, 1.0f, 0.5f),
								0.0f);
	}
};
...
__declspec(align(16)) struct ManipCost final sealed{
	...
	__declspec(noalias) __forceinline ManipCost(int _n,
																							bool _variableLen,
																							float _targetX,
																							float _targetY,
																							float _minTheta) noexcept
			: n(_n),
	...
	__declspec(noalias) __forceinline float operator()(
			...
			if (v > 0.0f) {
				...
				const float arg =
						fmaf(scale,
								 ...
								 0.0f);
				const float exp2_val =
						fmaf(arg,
								 fmaf(arg,
											fmaf(arg,
													 fmaf(arg, 0.00833333377f, 0.0416666679f),
													 0.16666667f),
											0.5f),
								 1.0f);
...
static __declspec(noalias) __forceinline void HitTest2D_analytic(
	...
	unsigned idx = (unsigned)(norm * (float)scale);
...
static __declspec(noalias) __forceinline float FindX2D_analytic(
	...
	while (l < levels) {
		...
		const unsigned tr =
			(unsigned)((clamped_px > mx) & (clamped_py > my));
		const unsigned tl =
			(unsigned)((clamped_px < mx) & (clamped_py > my));
		const unsigned dl =
			(unsigned)((clamped_px < mx) & (clamped_py < my));
		const unsigned none =
			(unsigned)(1u ^ (tr | tl | dl));
		...
		const unsigned dx = dd >> 1;
		const unsigned dy = dd & 1u;
		x0 += dx ? sx : 0.0f;
		y0 += dy ? sy : 0.0f;
...
#pragma loop ivdep
		...
		while (s > 0) {
			...
			const int j = (int)(st % (unsigned)(s + 1));
...
#pragma loop ivdep
		while (d < dim) {
			...
			const float pos =
				((float)stratum + u) / (float)ns;
			const int pd = map.perm[d];
			const float lo = map.low[pd];
			const float hi = map.high[pd];
...
static __declspec(noalias) __forceinline int generate_heuristic_seeds(
...

	{
		...
		const float phi =
			...
			? atan2f(ty, tx)
			: 0.0f;
		const float len = fminf(fmaxf((1.0f / (float)n) * rho, 0.5f), 2.0f);
...
#pragma loop ivdep
		...
		if (VL) {
			...
			while (j < n) {
				float si;
				FABE13_SIN(fmaf(1.5f, (float)j, 0.0f), si);
	...
	const int lhs_count =
		generate_lhs_seeds_lite(map,
			dim,
			S + total_seeds * stride,
			stride,
			seed);
...
static __declspec(noalias) void agp_run_branch_mpi(
	...
	auto t_to_idx = [&](float t) -> unsigned long long {
		unsigned long long idx =
			(unsigned long long)fmaf(t, (float)map.scale, 0.0f);
		return idx;
		};
	...
	float exp_arg_threshold =
		fmaf(1.0f /
			fmaf(dim_f,
				fmaf(dim_f,
					fmaf(dim_f,
						fmaf(dim_f,
							fmaf(dim_f, -0.2f, 0.25f),
							-0.33333333f),
						0.5f),
					-1.0f),
				dim_f),
			0.415888308336f,
			0.0f);
	const float log_arg_threshold =
		fmaf(dim_f, 0.1f, -0.105f);
	const float adaptive_coeff_addition_threshold =
		fmaf(
			fmaf(exp_arg_threshold,
				fmaf(exp_arg_threshold,
					fmaf(exp_arg_threshold,
						fmaf(exp_arg_threshold,
							0.00833333377f,
			...
			fmaf(log_arg_threshold,
				...
				log_arg_threshold),
			0.17814618538f);
	float adaptive_coeff_threshold =
		adaptive_coeff_addition_threshold + 1.0f;
	...
	const float C_dim =
		fmaf(log_argument,
			...
			log_argument) -
		B_dim;
	const float adaptive_coeff_addition =
		fmaf(C_dim,
			fmaf(C_dim,
				fmaf(C_dim,
					fmaf(C_dim,
	...
	auto evalAt = [&](float t) -> float {
		...
		const float step_arg =
			fmaf(-dim_f, 0.825f, 3.3f);
		float eta =
			fmaf(fmaf(step_arg,
				fmaf(step_arg,
					fmaf(step_arg,
		...
		if ((f < fmaf(bestF, 2.03f - adaptive_coeff, 0.0f) && p > 0.55f) ||
			(f < fmaf(bestF, adaptive_coeff, 0.0f) &&
				((p > 0.7f && !(it % 3)) || p > 0.9f))) {
			float acc = 0.0f;
			int ii = 0;
			while (ii < n) {
			...
			while (k >= 0) {
				const float Lk =
					cost.variableLen ? q_local[n + k] : 1.0f;
			...
			int stepI = 0;
			const float adaptive_window =
				fmaf(1.0f / fmaf(bestF, adaptive_coeff_threshold, 0.0f),
					fmaf(bestF, adaptive_coeff_threshold, -f),
					0.0f);
			const float curse_dim_arg =
				fmaf(dim_f, 0.825f, 0.0f);
			const int threshold_iters_grad =
				(int)fmaf(
					fmaf(adaptive_window,
						adaptive_window,
						1.0f),
					fmaf(fmaf(curse_dim_arg,
						fmaf(curse_dim_arg,
							fmaf(curse_dim_arg,
								fmaf(curse_dim_arg,
									0.00833333377f,
			...
			const int check_interval =
				(int)fmaf(1.0f /
					log2f(fmaf(dim_f, 0.5f, 1.0f)),
					(float)(threshold_iters_grad >> 2),
					0.0f);
...
#pragma loop ivdep
				while (i < n) {
...

					{
						...
						const float v =
							fmaf(ai, 1.0f, -cost.minTheta);
						if (v > 0.0f) {
							const float scale_arg =
								fmaf(2.0f / (cost.minTheta + 1.0e-6f),
									fmaf(v,
										0.69314718055994530941723212145818f,
										0.0f),
									0.0f);
							const float exp_val =
								fmaf(scale_arg,
									fmaf(scale_arg,
										...
										0.5f),
									1.0f);
							const float dpen_dtheta =
								fmaf(cost.sharpW,
									fmaf(exp_val,
										0.69314718055994530941723212145818f *
										(2.0f / (cost.minTheta + 1.0e-6f)),
										0.0f),
									copysignf(1.0f, q_local[i]));
					...
					{
						const float tsg =
							fmaf(-q_local[i], cost.archBiasK, 0.0f);
						...
						const float sig = 1.0f /
							fmaf(exp_val, 1.0f, 1.0f);
						gpen = fmaf(
							-fmaf(cost.archBiasW, cost.archBiasK, 0.0f),
							sig,
							gpen);
					}

					const float g =
						fmaf(fmaf(dx, -sum_s[i],
							fmaf(dy, sum_c[i], 0.0f)),
							1.0f / dist,
							gpen);
					q_try[i] = fmaf(-eta, g, q_local[i]);

					const float lo =
						(i == 0) ? -1.0471975511965977461542144610932f
...
#pragma loop ivdep
				...
				if (f2 < f) {
					...
					eta = fmaf(
						eta,
						fmaf(10.0f / sqrtf(dim_f),
							1.0f / (1.0f + rel_improvement) *
							rel_improvement,
							1.0f),
						0.0f);
				}
				else {
					const float caution_coeff =
						5.0f / sqrtf(dim_f) * rel_improvement;
					eta = fmaf(
						eta,
						fmaf(caution_coeff,
							fmaf(caution_coeff,
				...
				if (steps_since_check == check_interval) {
					if (fmaf(-1.0f / f_at_check_start, f, 1.0f) <
						1e-3f) {
						break;
					}
					f_at_check_start = f;
		...
		if (f < bestF) {
			if (0.95f < p) {
				const int last = n - 1;
				const float lo =
					(last == 0) ? -1.0471975511965977461542144610932f
					: -2.6179938779914943653855361527329f;
				const float hi =
					2.6179938779914943653855361527329f;
	...
	const int seedCnt = generate_heuristic_seeds(
		cost,
		map,
		dim,
		seeds,
		32,
		(unsigned)fmaf((float)rank, 7919.0f, (float)seed));

	int i = 0;
	while (i < seedCnt) {
		const float* s =
			seeds + (size_t)fmaf((float)i, 32.0f, 0.0f);
		const float t_seed = map.pointToT(s);

		const float interval_size =
			(i < 3)
			? fmaf(0.0004f, (float)dim, 0.0f)
			: fmaf(
				fmaf(0.00031f, (float)dim, 0.0f),
				exp2f(
					(1.0f /
						(float)(seedCnt - 4)) *
					log2f(fmaf(0.00025f,
		...
		const float t1 =
			fmaxf(a, fmaf(-interval_size, 1.0f, t_seed));
		const float t2 =
			fminf(b, fmaf(interval_size, 1.0f, t_seed));
		...
		if (t2 > t1) {
			...
			if (i < 3) {
				I->R = fmaf(I->R,
					fmaf(0.01f, (float)dim, 0.85f),
					0.0f);
			}
			else {
				const float start_mult =
					fmaf(0.214f, (float)dim, 0.0f);
				const float end_mult =
					fmaf(0.174f, (float)dim, 0.0f);
				const float mult =
					fmaf(start_mult,
						exp2f((1.0f /
							(float)(seedCnt - 4)) *
	...
	while (k <= K) {
		...
		IntervalND* I =
			new IntervalND(prev_t, t, prev_f, f);
	...
	IntervalND* tail =
		new IntervalND(prev_t, b, prev_f, f_b);
	...
	float kickEveryDimf =
		fmaf(120.0f, exp2f(-0.05f * dim_f), 0.0f);
	if (kickEveryDimf < 60.0f) {
		kickEveryDimf = 60.0f;
	}
	const int kickEveryDim = (int)kickEveryDimf;

	float noImproveThrDimf =
		fmaf(80.0f, exp2f(-0.08f * dim_f), 0.0f);
	if (noImproveThrDimf < 30.0f) {
		noImproveThrDimf = 30.0f;
	}
	const int noImproveThrDim = (int)noImproveThrDimf;

	auto kickEveryByDim = [&](int d_) -> int {
		float z =
			fmaf(120.0f, exp2f(-0.05f * (float)d_), 0.0f);
		if (z < 60.0f) {
			z = 60.0f;
		}
		return (int)z;
	...
	auto noImproveThrByDim = [&](int d_) -> int {
		float z =
			fmaf(80.0f, exp2f(-0.08f * (float)d_), 0.0f);
		if (z < 30.0f) {
			z = 30.0f;
		}
		return (int)z;
	...
	while (it < maxIter) {
		...
		if ((it % kickEveryDim) == 0 &&
			no_improve > noImproveThrDim) {
			...
			while (ii < 2) {
				...
				const float t_seed =
					fminf(b,
						fmaxf(a,
							fmaf(off, 1.0f, t_best)));
				const float f_seed = evalAt(t_seed);

				IntervalND* J =
					new IntervalND(fmaf(-0.005f, 1.0f, t_seed),
						fmaf(0.005f, 1.0f, t_seed),
						f_seed,
						f_seed);
				...
				J->ChangeCharacteristic(
					fmaf(r_eff, Mmax, 0.0f));
		...
		exp_arg_threshold =
			fmaf(-exp_arg_threshold * p, 10.0f, 0.0f);
		adaptive_coeff_threshold =
			fmaf(fmaf(exp_arg_threshold,
				fmaf(exp_arg_threshold,
					fmaf(exp_arg_threshold,
						fmaf(exp_arg_threshold,
							0.00833333377f,
					...
					0.5f),
				1.0f),
				adaptive_coeff_addition_threshold,
				1.0f);
		...
		adaptive_coeff =
			fmaf(-fmaf(exp_arg,
				fmaf(exp_arg,
					...
					0.5f),
				1.0f),
				adaptive_coeff_addition,
				A_dim);

		const bool stagnation =
			(no_improve > 100) && (it > 270);

		const float exp_argument =
			fmaf(-0.06f, dim_f, 0.0f);
		const float exp2_exp_arg =
			fmaf(exp_argument *
				0.69314718055994530941723212145818f,
				fmaf(exp_argument *
					0.69314718055994530941723212145818f,
		...
		const float A =
			fmaf(64.0f, exp2_exp_arg, 200.0f);
		const float B =
			fmaf(67.0f, exp2_exp_arg, 210.0f);
		const int T =
			(int)fmaf(-(exp2_exp_arg - 1.0f), A, B);
		...
		float tNew =
			step(m, x1, x2, y1, y2, dim_f, r_eff);
		...
		if (adaptive) {
			...
			if ((p > 0.7f && !(it % 3) && dmax < 0.7f) ||
				p > 0.9f) {
				const float alpha = p * p;
				const float beta =
					fmaf(-alpha, 1.0f, 2.0f);

				const float MULT =
					(1.0f / dmax) * Mmax;
				const float global_coeff =
					fmaf(MULT, r_eff, -MULT);
				const float GF = beta * global_coeff;

				L->ChangeCharacteristic(
					fmaf(GF, len1, fmaf(L->M, alpha, 0.0f)));
				Rv->ChangeCharacteristic(
					fmaf(GF, len2, fmaf(Rv->M, alpha, 0.0f)));

				const size_t sz = H.size();
				RecomputeR_AffineM_AVX2_ND(
					H.data(),
					sz,
					GF,
					alpha);
		...
		const float interval_len = top->diam;

		if (interval_len < eps || it == maxIter) {
		...
		if (!(it % T)) {
			...
			const size_t iterations =
				std::bit_width((size_t)(world - 1));
			bool active = true;
			const bool invert_T =
				((int)fmaf((float)exchange_counter_T, 1.0f, 1.0f)) & 1;
		...
		if (!(it % 500)) {
			...
			memcpy(out.bestQ,
				bestQ.data(),
				bestQ.size() * sizeof(float));

			const size_t iterations =
				std::bit_width((size_t)(world - 1));
			bool active = true;
			const bool invert_T =
				((int)fmaf((float)exchange_counter_500, 1.0f, 1.0f)) & 1;
		...
		while (g_world->iprobe(boost::mpi::any_source, 0)) {
			...
			while (ii < mX.count) {
				...
				if (ex > sx) {
					...
					IntervalND* inj =
						new IntervalND(sx, ex, y1i, y2i);
					inj->i1 = t_to_idx(sx);
					inj->i2 = t_to_idx(ex);
					inj->diam =
						map.block_diameter(inj->i1, inj->i2);
					inj->compute_span_level(map);
					inj->set_metric(inj->diam);
					update_pockets_and_Mmax(inj);
					inj->ChangeCharacteristic(
						fmaf(r_eff, Mmax, 0.0f));
					...
					if (inj->R >
						fmaf(adaptive_coeff, topH->R, -0.03f)) {
						const float poly =
							fmaf(p *
								0.69314718055994530941723212145818f,
								fmaf(p *
						...
						const float kf =
							(no_improve > 100 && it > 270)
							? fmaf(0.5819767068693265f,
								poly,
								0.3f)
							: fmaf(0.3491860241215959f,
								poly,
								0.6f);
		...
		while (g_world->iprobe(boost::mpi::any_source, 2)) {
			...
			if (bm.bestF < fmaf(bestF, 1.15f, 0.0f)) {
				...
				const float t1 =
					fmaxf(a, fmaf(-0.001f, 1.0f, t_best));
				const float t2 =
					fminf(b, fmaf(0.001f, 1.0f, t_best));
				...
				if (t2 > t1) {
					...
					I->ChangeCharacteristic(
						fmaf(r_eff, Mmax, 0.0f));
					I->R =
						fmaf(I->R, 2.03f - adaptive_coeff, 0.0f);
...
void AGP_Manip2D(int nSegments,
	...
	const int dim =
		nSegments + (variableLengths ? nSegments : 0);

	g_mc.permCache.resize((size_t)dim);
	int i = 0;
	while (i < dim) {
		g_mc.permCache[i] = i;
		++i;
	...
	while (i > 0) {
		...
		s ^= s >> 17;
		s ^= s << 5;
		const unsigned j = s % (unsigned)(i + 1);
		std::swap(g_mc.permCache[i], g_mc.permCache[j]);
		--i;
	}

	g_mc.invMaskCache.resize((size_t)dim);
	int k = 0;
	while (k < dim) {
		s ^= s << 13;
		s ^= s >> 17;
		s ^= s << 5;
		g_mc.invMaskCache[k] = (unsigned long long)s;
		++k;
	}
	...
	while (i < nSegments) {
		low.emplace_back(
			i == 0
			? -1.0471975511965977461542144610932f
			: -2.6179938779914943653855361527329f);
		high.emplace_back(
			2.6179938779914943653855361527329f);
		++i;
	...
	const ManipCost cost(nSegments,
		variableLengths,
		targetX,
		targetY,
		minTheta);
	...
	const int levels0 =
		(int)fminf((float)peanoLevels, 8.0f);
	const int maxIter0 =
		(int)fmaf((float)maxIterPerBranch, 0.2f, 0.0f);

	const MortonND map0(dim, levels0, low.data(), high.data(), g_mc);

	std::vector<IntervalND*,
		boost::alignment::aligned_allocator<IntervalND*, 16u>>
		H_coarse;
	std::vector<float, boost::alignment::aligned_allocator<float, 16u>>
		bestQ_coarse;
	...
	const float base_M_prior_factor =
		fmaf(2.0f, (float)nSegments,
			variableLengths ? 1.41421356f : 0.0f);
	float M_prior =
		fmaf(base_M_prior_factor,
			ldexpf(1.0f, -levels0),
			0.0f);

	agp_run_branch_mpi(map0,
		cost,
		maxIter0,
		r,
		adaptiveMode,
	...
	if (levels0 < peanoLevels) {
		...
		while (ci < H_coarse.size()) {
			...
			IntervalND* I =
				new IntervalND(C->x1, C->x2, f1, f2);
			I->i1 = (unsigned long long)fmaf(
				C->x1, (float)map1.scale, 0.0f);
			I->i2 = (unsigned long long)fmaf(
				C->x2, (float)map1.scale, 0.0f);
			I->diam =
				map1.block_diameter(I->i1, I->i2);
			I->set_metric(I->diam);
		...
		agp_run_branch_mpi(map1,
			cost,
			(int)fmaf((float)maxIterPerBranch,
				1.0f,
				-(float)maxIter0),
			r,
			adaptiveMode,
	...
	best.bestX = bx;
	best.bestY = by;
	best.dim = (unsigned)bestQ.size();
	memcpy(best.bestQ,
		bestQ.data(),
		(size_t)best.dim * sizeof(float));
	...
	if (rank == 0) {
		*out_bestQLen = (size_t)best.dim;
		*out_bestQ =
			(float*)CoTaskMemAlloc(sizeof(float) *
				(*out_bestQLen));
		memcpy(*out_bestQ,
			best.bestQ,
			sizeof(float) * (*out_bestQLen));
...
AgpInit(int peanoLevel,
	...
	if (world_size == 4) {
		new (&gActiveMap)
			Peano2DMap(peanoLevel, a, b, c, d, rank & 3);
	}

	g_mc.baseSeed =
		(unsigned)fmaf(0x9E3779B9u,
			(float)rank,
			0x9E3779B9u);
	...
	if (rank == 0) {
		...
		path.attr("insert")(0,
			pybind11::str(
				exeDir + "\\env\\Scripts"));
		path.attr("insert")(0,
			pybind11::str(exeDir + "\\env"));
		path.attr("append")(pybind11::str(exeDir));

		pybind11::module_::import("warnings")
			.attr("filterwarnings")("ignore");

		g_pyOptimizerBridge =
			new pybind11::module_(
...
static __declspec(noalias) __forceinline float HillFunc(
	...
	const float st_ang =
		6.28318530717958647692f * x;
	while (j < 14) {
		ang[j] = st_ang * (float)(j + 1);
		++j;
	}
...
void AGP_1D(float global_iterations,
	...
	const float threshold_03 =
		0.3f * initial_length;
	const float inv_threshold_03 =
		1.0f / threshold_03;
	...
	std::vector<Interval1D*,
		boost::alignment::aligned_allocator<Interval1D*, 16u>>
		R;
	...
	R.emplace_back(
		new Interval1D(a, b, start_val, best_f, 1.0f));

	float Mmax = R.front()->M;
	float m = r * Mmax;

	while (true) {
		const float new_point =
			step(m,
				x_Rmax_1,
				x_Rmax_2,
				y_Rmax_1,
		...
		if (++counter == (int)global_iterations ||
			interval_len < epsilon) {
			Extr.emplace_back((float)counter);
			Extr.emplace_back(interval_len);

			*out_len = Extr.size();
			*out_data = (float*)CoTaskMemAlloc(
		...
		Interval1D* curr =
			new Interval1D(new_x1,
				new_point,
				pro->y1,
				new_value,
				1.0f);
		Interval1D* curr1 =
			new Interval1D(new_point,
				new_x2,
				new_value,
				pro->y2,
				1.0f);
		...
		if (mode) {
			...
			if ((threshold_03 > dmax && !(counter % 3)) ||
				10.0f * dmax < initial_length) {
				if (currM > Mmax) {
					Mmax = currM;
					m = r * Mmax;
				}
				...
				const float progress =
					fmaf(-inv_threshold_03, dmax, 1.0f);
				...
				curr->ChangeCharacteristic(
					fmaf(GF, len1, curr->M * alpha));
				curr1->ChangeCharacteristic(
					fmaf(GF, len2, curr1->M * alpha));

				RecomputeR_AffineM_AVX2_1D(
					R.data(),
					r_size,
					GF,
					alpha);
				...
				if (currM > Mmax) {
					if (currM < 1.15f * Mmax) {
						...
						curr->ChangeCharacteristic(m);
						curr1->ChangeCharacteristic(m);
						RecomputeR_ConstM_AVX2_1D(
							R.data(),
							r_size,
							m);
						std::make_heap(R.begin(),
							R.end(),
							ComparePtr1D);
					}
				}
...
void AGP_2D(float N,
	...
	const float inv_divider =
		ldexpf(1.0f, -((gActiveMap.levels << 1) + 1));
	const float x_addition =
		(b - a) * inv_divider;
	const float y_addition =
		(d - c) * inv_divider;
	...
	const float start_val =
		(rank % 3)
		? RastriginFunc(true_end, d - y_addition)
		: RastriginFunc(true_start, c + y_addition);
	float best_f =
		(rank % 2)
		? RastriginFunc(true_start, d - y_addition)
	...
	std::vector<float, boost::alignment::aligned_allocator<float, 16u>> Extr;
	std::vector<Interval1D* __restrict,
		boost::alignment::aligned_allocator<
		Interval1D* __restrict, 16u>>
		R;

	Extr.reserve((size_t)global_iterations << 2u);
	R.reserve((size_t)global_iterations << 1u);

	R.emplace_back(new Interval1D(true_start,
		true_end,
		start_val,
		best_f,
		2.0f));
	...
	while (true) {
		const float interval_len =
			x_Rmax_2 - x_Rmax_1;
		const bool stagnation =
			no_improve > 100 && counter > 270;
		const float p =
			fmaf(-1.0f / initial_length, dmax, 1.0f);
		...
		while (g_world->iprobe(boost::mpi::any_source, 0)) {
			...
			while (ii < mX.count) {
				...
				if (ex > sx) {
					Interval1D* __restrict injected =
						new Interval1D(sx,
							ex,
							RastriginFunc(d2[0], d2[1]),
							RastriginFunc(d2[2], d2[3]),
							2.0f);
		...
		const bool want_term =
			interval_len < epsilon ||
			counter == (int)global_iterations;
		...
		if (want_term) {
			if (!rank) {
				...
				*out_len = Extr.size();
				*out_data =
					(float* __restrict)CoTaskMemAlloc(
						sizeof(float) * (*out_len));
				memcpy(*out_data,
					Extr.data(),
					sizeof(float) * (*out_len));
		...
		const float new_value =
			RastriginFunc(new_x1_val, new_x2_val);
		...
		Interval1D* __restrict curr =
			new Interval1D(segment_x1,
				new_point,
				intermediate->y1,
				new_value,
				2.0f);
		Interval1D* __restrict curr1 =
			new Interval1D(new_point,
				segment_x2,
				new_value,
				intermediate->y2,
		...
		if (mode) {
			...
			if ((threshold_03 > dmax && !(counter % 3)) ||
				...
				const float MULTIPLIER =
					(1.0f / dmax) * Mmax;
				const float global_coeff =
					fmaf(MULTIPLIER, r, -MULTIPLIER);
				const float GLOBAL_FACTOR =
					betta * global_coeff;

				curr->ChangeCharacteristic(
					fmaf(GLOBAL_FACTOR, len1, curr->M * alpha));
				curr1->ChangeCharacteristic(
					fmaf(GLOBAL_FACTOR, len2, curr1->M * alpha));
...
__declspec(align(16)) struct RunParams final sealed{
	...
	void serialize(Archive& ar, unsigned int) {
		ar& nSegments& varLen& minTheta& tx& ty& levels& maxIter& r&
				adaptive& eps& seed& baseLength& stretchFactor;
	}
};
...
void RunPythonOptimization(
	...
	if (backend && strcmp(backend, "optuna") == 0) {
		result = (*g_pyOptimizerBridge)
			.attr("run_optuna")(n_seg,
				var_len,
				min_theta,
				tx,
				ty,
	...
	*out_micros = result["TIME"].cast<float>();
	...
	std::vector<float> angles_vec =
		result["ANGLES"].cast<std::vector<float>>();
	std::vector<float> lengths_vec =
		result["LENGTHS"].cast<std::vector<float>>();

	const size_t n = (size_t)n_seg;
